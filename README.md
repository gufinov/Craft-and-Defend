...
`START.cmd` options: no argument prepares the provenance-matched export and plays (the main menu offers the real game and CoasterCraft); `START.cmd sandbox` = CoasterCraft plus the premade demo tracks with the data root `artifacts\coaster-sandbox`; `START.cmd coastercraft` = straight into CoasterCraft New (data root `artifacts\coastercraft`); `START.cmd build` rebuilds the Windows export now; `START.cmd stop` closes every running game window; `START.cmd help` prints this list. The old `START_GAME`, `START_COASTER_SANDBOX`, `START_COASTERCRAFT`, `START_F0`, `STOP_GAME`, `BUILD_WINDOWS`, `BUILD_WINDOWS_F0` and `VIEW_NAVIGATION_SPIKE` launchers are gone.
