...
func _build_loop_track_visual(parent: Node3D, record: Dictionary, joined: Array[Vector3i], node_name: String = "LoopTrack") -> void:
	...
	for cell: Vector3i in joined:
		...
		if round and str(other.get("entity_id", "")) == CoasterRails.SLOPE:
			...
			# tool's piece): a straight to the shared meeting point - along
			# the curve first when it ends on the shared face (auto-shaped
			# elbows and lane shifts).
			var face_end := _curve_face_end(record, other)
			if face_end >= 0.0:
				var own_t := TrackCurve.piece_t(record)
				for section in range(1, LOOP_ARC_SECTIONS + 1):
					points.append(TrackCurve.point(curve, own_t + (face_end - own_t) * float(section) / float(LOOP_ARC_SECTIONS)))
			else:
