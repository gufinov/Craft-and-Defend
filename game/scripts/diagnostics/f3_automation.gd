...
func _run_phase1() -> void:
	...
	_record("T26_MIDJOB_SAVED", edit_a.get("ok", false) and placed.get("ok", false) and started.get("ok", false) and is_equal_approx(remaining, 3.0) and saved_a.get("ok", false), "slot A saves a three-second remaining furnace job", {"remaining": remaining, "save": saved_a})
...
func _run_phase2() -> void:
	...
	var before := app.session.inventory.count("iron_ingot")
	app.session.workstations.advance(2.9, false)
	var before_finish := app.session.inventory.count("iron_ingot")
	app.session.workstations.advance(0.11, false)
	var completed := app.session.inventory.count("iron_ingot")
	app.session.workstations.advance(10.0, false)
	var after_extra := app.session.inventory.count("iron_ingot")
	_record("T26_MIDJOB_CONTINUE", is_equal_approx(remaining, 3.0) and before == 0 and before_finish == 0 and completed == 1 and after_extra == 1 and app.session.workstations.jobs.is_empty(), "mid-job restart preserves remaining time and completes exactly once", {"remaining": remaining, "before": before, "before_finish": before_finish, "completed": completed, "after_extra": after_extra})
	await _dispose_session()
	app.saves.select_slot("b")
	app._show_main_menu()
	app._on_continue_pressed()
