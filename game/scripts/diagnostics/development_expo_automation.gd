...
func _run_gate() -> void:
	...
	# is `siege_defense.advance` doing its ordinary work, and the Battlefield is
	# a real `CoreDefenseService` drill. Everything that had to be read as built
	# has been read by now, so the simulation is handed back to the game.
