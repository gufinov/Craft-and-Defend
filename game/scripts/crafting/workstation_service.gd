...
func restore(data: Dictionary, world_query: Callable) -> Dictionary:
	...
	for value in data.get("stations", []):
		...
		var reserved := footprints.try_reserve(str(record.instance_id), record.anchor, _vector_list(definition.occupied_offsets), int(record.get("rotation_quarters", 0)), world_query, AABB(), restore_support)
		if not reserved.get("ok", false):
