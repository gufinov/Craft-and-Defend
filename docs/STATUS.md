...
**STATUS:** **ROUND-2 BUILD + ROUND-3/4/5 FIXES — CANDIDATE PASS ON THE EXPORT (PCK `5630dca1…`, branch `feature/p3d-tools-world-feedback`, pushed); OWNER RE-PLAYTEST PENDING; NOT MERGED TO `main`.**

**DONE (newest first):**
- **Round-6 fixes** (`5ce8c5a`, `8d0c301`): wall lanterns hang on walls (torches on ground or wall); rails re-shape into corners / T / crossroads and kettles ride corners but keep straight at junctions; stuck raiders back up and sidestep before re-planning; recipes that overflowed the 3×3 grid (mk2, cannon, core) fixed with a validator rule. Side project **coaster rails** running in worktree `worktrees\coaster-rails` (branch `feature/coaster-rails`, not merged). T155.
- **Coaster rails side project** (branch `feature/coaster-rails`, on top of `5ce8c5a`, NOT merged): 45° `rail_slope`, the `rail_loop` drag tool (Shift loop, X / C radius) and a `mine_cart` that rides slopes and loops; kettles keep their flat-rail behaviour and also climb slopes; T160–T163, `TEST_COASTER_RAILS.cmd`. See [Coaster rails](COASTER_RAILS.md) and [its evidence](evidence/COASTER_RAILS.md); the Workbench book is four pages there.
...
**TEST:** Static PASS (190 links; 60/60). Exported PCK `5630dca1…`: all twelve runners PASS (EXE `4ac12872…`): all twelve runners PASS (T123 real-click test flaked once, passed on rerun) — `TEST_P4_SIEGE_UNITS` (T130–T136, T141–T143, T147, T151, T152), `TEST_P4_ASSETS` (T148–T150), `TEST_P4_ENEMY_UNITS`, `TEST_P4_WEAPON_PANEL`, `TEST_P4_RESOURCES`, `TEST_P3C`, `TEST_P3G`, `TEST_P3D`, `TEST_P3F`, `TEST_P3K`, `TEST_P3E`, `TEST_P3H`. Editor gates F0, F1, F2, F3, P1, P2, P3, P3B (all modes), castle kit PASS. Owner playtest NOT RUN.
