...
**STATUS:** **ROUND-2 BUILD + ROUND-3/4/5/6 FIXES + COASTER RAILS + COASTER CAR / HERO MERGED, HUD CORNER MINIMAP — CANDIDATE PASS ON THE EXPORT (PCK `b51468c9…`, branch `feature/p3d-tools-world-feedback`, pushed); OWNER RE-PLAYTEST PENDING; NOT MERGED TO `main`.**

**DONE (newest first):**
- **CoasterCraft cards 6 + 8** (branch `feature/coaster-autoclear-supports`): pause-menu **Track auto-clear** setting (track tools mine natural terrain in their way, amber ghost, drops to the pack) and **automatic trestle supports** under floating loop pieces (stone posts with gold studs and braces, skipped over the top and where the ring stacks); T179-T181. See [Coaster rails](COASTER_RAILS.md#track-auto-clear-coastercraft-card-6-2026-09-20). Not yet merged.
...
**TEST:** Static PASS (validator 49 items / 42 recipes; 60/60). Exported PCK `b51468c9…` (EXE `4ac12872…`): all fourteen runners PASS — `TEST_COASTER_CAR` (T164–T167), `TEST_COASTER_RAILS` (T160–T163), `TEST_P4_SIEGE_UNITS`, `TEST_P4_ASSETS`, `TEST_P4_ENEMY_UNITS`, `TEST_P4_WEAPON_PANEL`, `TEST_P4_RESOURCES`, `TEST_P3C`, `TEST_P3G`, `TEST_P3D`, `TEST_P3F`, `TEST_P3K`, `TEST_P3E`, `TEST_P3H` (sweep log in scratch `runall.log`, evidence in the newest `artifacts/manual-*` per runner). Owner playtest NOT RUN.

**LIMITATIONS/FAILURES:** Water is walk-through (no swimming). Trolls shoot through walls. The enemy base is just the red core on a clearing (no camp, no patrols); attacking it is not yet a win condition. The march ignores player structures until it is 20 cells from the core. No corkscrew rail yet (loop only). Diagnostic fixtures use the fixed seed 41026 and an empty starting pack.
...
**GIT/REPRODUCIBILITY:** Worktree `D:\CODEX\Craft_and_Defend\worktrees\p3d-tools-world-feedback`, branch `feature/p3d-tools-world-feedback`, pushed; `feature/p4-core-and-lights` (`e8ffeb5`) and `feature/coaster-rails` (`fe1be67`…, pushed) and `feature/coaster-car-and-hero` (`a230867`) merged. Engine `4.6.stable.custom_build.89cea1439`; EXE SHA-256 `4ac128729e86108904e6d038d161322404d42d71892b1cc0dca751039aa7e4b2`; PCK SHA-256 `b51468c96ba513395f23bdf958849f6d2b4e57bb3543d4656dd9254f54c02a0c`. Evidence roots: the newest `artifacts/manual-*` folder per runner from the 2026-09-19 sweep (scratch `runall.log`).
