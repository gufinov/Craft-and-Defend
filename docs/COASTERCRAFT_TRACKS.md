...
**Card 1 is in** (2026-09-20): `game/scripts/rails/track_curve.gd` (`TrackCurve`: `make_*`, `point`, `tangent`, `up_at`, `nearest_t`, `cells`, `pieces`, `piece_t`). A record with `curve` + `t0`/`t1` rides the curve (`CoasterRails.ride_point`), joins only its recorded joints (`connections`), leans by `TrackCurve.up_at` (`CoasterCartService._up_at`) and draws with `GameSession._build_loop_track_visual` (same-curve neighbours are followed along the curve). The true loop is `TrackCurve.make_helix` laid by `TrackCurve.pieces`; the classic loop ring still uses `loop_center` (fine, it is the fallback). **Cards 2 and 3 are in** (2026-09-20, `feature/coaster-smooth-switcher`): the smooth switch (then item `rail_bend`) and the Crossing (`rail_cross`) - `CoasterRails.bend_layout` / `cross_layout`, `pair_for` for the crossing's two-curve shared cells, `InteractionService` modes `smooth_bend` / `rail_cross` (Shift-aim = length and lanes), T170-T172; see the section in `COASTER_RAILS.md`. **Rail Switch is the smooth switcher** (2026-09-20, `feature/coaster-smooth-rail-switch`, owner: "the Rail Switch should be updated to be smooth"): `rail_bend` was removed and `rail_switch` (Workbench 208, -> 8, stack 64) now carries `coaster_tool: "bend"`, default 4 long x 1 lane; the four-piece `lane_switch` drag is gone (the blocky `rail_switch` pieces remain only as the loop element's entry / exit runs). T168 / T170 / T172 / T182. Cards 4–6 and 8 are open for agents.
...
| **Smooth lane switcher** (done: it IS the Rail Switch) | s_bend: lateral shift of *n* lanes over length *L*, cosine profile | drag along = length, sideways = lanes | replaced the 4-block switcher's kinked diagonal; the block pieces survive only inside the loop element |
