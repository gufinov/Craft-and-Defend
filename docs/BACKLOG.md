...
## P3I — Furnace auto-processing (CANDIDATE 2026-09-18)

**Recorded candidate result:** implemented per [the P3I contract](P3I_FURNACE_AUTO_PROCESSING.md); T107 added to the P3G gate. Owner direction 2026-09-18: a Furnace with Raw Input and Fuel present starts its job without a manual press and continues while the player is away from the menu (live processing already advances from the session; manual start is the gap). Keep the persistent-container contract, the counted-fuel rule, the single timed job and lossless cursor-stack close. Extend T87/P3G furnace tests with an unattended raw+fuel deposit completing exactly one output per operation. **Boundary:** no hopper/automation blocks, no chest runtime, no recipe changes.

## P3J — drag building (CANDIDATE 2026-09-18)

**Recorded candidate result:** implemented per [the P3J contract](P3J_DRAG_BUILDING.md); T108/T109 added to the P3D suite. Owner direction 2026-09-18: with a placeable block held, right-press-and-drag previews a row along the dragged axis; dragging upward previews a column; both together preview a wall; release commits one multi-cell placement transaction limited to the blocks actually carried. Pressing left while still holding right cancels with nothing built or consumed. Preview uses the existing placement validation per cell without reserving cells; commit repeats validation and is all-or-nothing per the placement/inventory contracts. No post-build undo in this slice. Add T-cases for row/column/wall commit, insufficient stock, partial invalid cells and cancel. **Boundary:** no undo stack, no entity (multi-cell footprint) dragging, no free-floating placement.

## P3K — parapet auto-connect and first blueprints (SLICE 1 CANDIDATE 2026-09-18)

**Recorded candidate result (slice 1):** data contract, stamp engine, sockets, catalogue (foundation, segment, caps 4/6/8, wall) and T110–T113 per [the P3K contract](P3K_BLUEPRINTS.md); UI entry point, parapet block and Tower Platform retirement remain. Owner decision 2026-09-18: castle pieces are blueprints that stamp ordinary blocks (see [design direction §9](DESIGN_DIRECTION_2026-09-18.md)). Add a parapet block whose merlons open where a walkway or floor block meets it (fence-style neighbour rule), and a blueprint model (block list relative to an anchor, typed `top`/`side` sockets, ghost preview, summed block cost, all-or-nothing stamp) with foundation, tower segment with spiral stair, caps 4×4/6×6/8×8, wall segment and stair-to-walk. Retire the Tower Platform entity with save refusal or migration per the persistence contract. **Boundary:** no player templates yet, no build wheel, no siege changes.

## P4a-1 — siege motion (CANDIDATE 2026-09-19)

Catapult modelled from the owner reference (2×4 footprint) with a `SiegeTurret` turntable that turns to face the target at `TURN_RATE`, a `CatapultArm` hinge that whips on fire and winds back over the reload cooldown, the bucket stone visible only when loaded, and the shot launched from the bucket's real position. Ballista shares the turntable. T97 (identity), T98 (render) and T118 (motion) cover it. **Next:** P4a-2 weapon panel, P4a-3 chest + auto-reload supply, P4a-4 flame shot and fire — see [design direction §11](DESIGN_DIRECTION_2026-09-18.md).

## P4a-2 — weapon and chest panels (CANDIDATE 2026-09-19)

Right-clicking a placed Ballista or Catapult (station type `siege`) opens a weapon panel in the crafting modal shell: weapon stats, an Ammunition slot fed by select-then-click / Shift / double-click / drag (`siege_load`), Unload, Hold / Fire at will (`siege_set_stance`), a target filter (`siege_set_target_filter`), a supply readout of chests in range (`siege_supply`) and a munition legend. Right-clicking a Chest (station type `chest`) opens a 3 × 3 container grid with deposit / withdraw gestures (`container_deposit` / `container_withdraw`). Contract: [P4a-2 weapon and chest panels](P4A2_WEAPON_AND_CHEST_PANELS.md); evidence: [P4a-2 evidence](evidence/P4A2_WEAPON_AND_CHEST_PANELS.md); T121–T123 via `tools\runners\TEST_P4_WEAPON_PANEL.cmd`. **Boundary:** no auto-distribution beyond the service's nearest-chest reload, no ammunition trough entity, no cursor-stack pickup from the weapon or chest tiles.

## P4C — siege units and wave drills (CANDIDATE 2026-09-19)

Four machines from the owner's reference images as ordinary siege stations on the P4a-2..4 service: remodelled Ballista (pedestal, slider draw/release, strings, loaded bolt), Turret Catapult (2×2, tower-platform socket, shared arm animation), Cannon (2×3 direct fire, cannonball, recoil and muzzle flash), Rail + Kettle (rails chain along wall tops; the kettle rides the chain to the raider and dumps hot oil that burns). New munitions `cannonball` and `hot_oil` (furnace recipe from logs). Every weapon targets its own nearest raider through a filter. Wave drills (P4D): `start_prototype({"raiders", "brutes", "spawn_distance"})`, brutes, per-raider routing and attack, splash across raiders, WON only when the wave is down; pause-menu **Start Wave Drill**. Contract: [P4C siege units and wave drills](P4C_SIEGE_UNITS_AND_WAVES.md); evidence: [P4C evidence](evidence/P4C_SIEGE_UNITS_AND_WAVES.md); T130–T136 via `tools\runners\TEST_P4_SIEGE_UNITS.cmd`. **Boundary:** raiders ignore rails and machines; no cannon damage to stone; the kettle only refills from chests within the generic supply radius.
## P4b-1 — resource distribution and gold (CANDIDATE 2026-09-19)

**Recorded candidate result:** ore distribution is one data table (`world.json → terrain.ores`: block, depth band, per-thousand frequency, cluster size) that reproduces the P1 coal/iron layout cell for cell and adds `gold_ore` (voxel 11, Iron Pick, depth ≥ 12, 5‰ clusters) plus `gold_ore`/`gold_ingot` items, the 8 s Furnace `gold_ingot` recipe, derived gold icons and validator/unit rules; T137–T140 PASS editor-headless per [the P4b contract](P4B_RESOURCE_DISTRIBUTION.md) and [evidence](evidence/P4B_RESOURCE_DISTRIBUTION.md). `generator_version` stays `terrain_p1_1` (gold is additive in untouched chunks). **Next:** export + `tools\runners\TEST_P4_RESOURCES.cmd`, owner playtest, then gold consumers (Foundry commission / market purchase, [design direction §5](DESIGN_DIRECTION_2026-09-18.md)) and original gold art. **Boundary:** no Foundry/market, no gold sink, no biome tables.

## P4F — enemy units: orc melee and troll ranged (CANDIDATE 2026-09-19)

The capsule raiders become two modelled units from the owner's reference images: the **orc melee** (green, bald, pointed ears, red left pauldron with iron plate, red loincloth, iron buckle, brown boots with iron toe caps, two grey cleavers with gold guards) is the basic `raider`; the brute is the same orc scaled 1.25 with a purple tint; the new **troll ranged** (`troll`: 28 hp, 5 damage, speed 2.4, 9-cell range, 2 s interval; blue-grey skin, topknot, tusks, red-and-fur shoulder pad, bracers, quiver, wooden crossbow) stops as soon as its route target (core or breach structure) is within range and shoots bolts from there. Walk bob, melee chop and crossbow kick are cheap per-frame maths and Tweens. `start_prototype({"trolls": n})`; wave/siege drills carry 1/3 trolls; `troll` target filter for weapons; balance keys `troll_health`, `troll_damage`, `troll_range`, `troll_attack_interval_seconds`. Contract: [P4F enemy units](P4F_ENEMY_UNITS.md); evidence: [P4F evidence](evidence/P4F_ENEMY_UNITS.md); T144–T146 via `tools\runners\TEST_P4_ENEMY_UNITS.cmd`. **Boundary:** no line of sight (trolls shoot through walls within range); the bolt is presentation, damage lands with the timer; trolls do not target machines or the player.

## P4E — the big world, enemy base and far attacks (CANDIDATE 2026-09-19)

World 320×48×384 (5× area) with random seeds per new game, mountains (rock above +12), lakes of translucent walk-through water, richer ore plus ore on the ground, and an enemy base clearing 150–200 cells from home with the red enemy core on it. Waves march from that base over a generator-height surface route (`SurfaceRouter`), ghost-walking unloaded ground, then hand over to the local planner near the defended core; near drills remain. Pathing: raiders no longer shove each other, re-plan when stuck, and spread randomly. The drill defends the placed Core of Power. Contract: [P4E](P4E_WORLD_AND_ENEMY_BASE.md); T151, T152. **Boundary:** no swimming, no line of sight for trolls, no camp structures or patrols at the enemy base, the march ignores player structures until the local area.

## P4G — Core of Power and light sources (CANDIDATE 2026-09-19)

Owner direction 2026-09-19 (eight reference renders): each player has a **Core of Power** (a rune monolith glowing blue for the player, a red orb core for the enemy) placed in the open on a stone slab; the goal is to protect it by building around it. Every new asset gets a **value and attributes** (role, light, mount, space, cost, defense, offense, notes) — a new per-entity `attributes` block that the validator enforces. Entities: `core_of_power` (3×3 base + centre column, 4 tall, 240, blue light, 8 castle stone + 2 gold ingot — crafted because the new-game inventory has no starting grant), `enemy_core` (same footprint, 400, red, hidden item with no recipe, placed by the game), `torch` (flickering), `wall_lantern`, `post_lantern` (3 tall), `campfire` (3×3, flickering, never burns out, no ignition), `light_block_blue` / `light_block_red` (block-sized). Each model adds one `OmniLight3D` from its attributes; **lights never go out**. Owner renders are the card icons (black backdrops keyed out). Contract and attribute sheet: [P4G](P4G_CORE_AND_LIGHTS.md); evidence: [P4G evidence](evidence/P4G_CORE_AND_LIGHTS.md); T148–T150 via `tools\runners\TEST_P4_ASSETS.cmd`. **Boundary:** no wall/ceiling side-face mounting (everything stands on a solid top); the wave drill is not yet retargeted to the placed core (separate card); no enemy camp places the red core; lights have no effect on enemies.

## Coaster car and hero (CANDIDATE 2026-09-19)

Owner request (2026-09-19): speed up and ride the roller coaster in a roller coaster car with 1–9 speed keys, and create a character. Delivered on `feature/coaster-car-and-hero`: `coaster_car` (recipe 207, owner-art icon and primitive model), `CoasterRide` (Shift boards a parked car, 1–9 = cells per second, Shift / Escape leaves beside the car, three-quarter chase camera, ride survives pause, save stores the player beside the car), `HeroModel` (unarmoured / armoured from the owner's art, walk cycle, seated pose) seated in the car and visible on foot in third person (**V**), pause-menu **Hero: Armour on/off** persisted in `settings.cfg`. Contract: [Coaster car and hero](COASTER_CAR_AND_HERO.md); evidence: [Coaster car and hero evidence](evidence/COASTER_CAR_AND_HERO.md); T164–T167 via `tools\runners\TEST_COASTER_CAR.cmd`. **Deferred:** momentum / gravity on the track; camera collision for both chase cameras; a first-person seat view; passengers and idle animation; earning the armour (currently a cosmetic toggle); rebindable V; a car repair path (Shift boards instead); saving the ride itself; owner playtest of the ride feel through the loop.

## Coaster rails — side project (CANDIDATE 2026-09-19)

Owner idea (2026-09-19): rails that climb 45 degrees so mine carts can go up mountains, and a roller-coaster option drawn by click-and-drag with keys held (equip a loop rail, drag the lead-in, hold Shift to start a loop, X / C resize it, release to lay the loop with a flat exit; DNA strands and corkscrews later). Delivered as a modular side feature (every new file carries a `coaster` name): `rail_slope` (1×1, rises one cell toward its front, W / R turn it), `rail_loop` (the loop tool; floating pieces, no support) and `mine_cart` (rides the whole track end to end at 3 cells/s, no gravity) with recipes 204–206; `CoasterRails` mutual-joint chain model that the kettle's `_rail_chain` now uses (flat chains identical; kettles also climb slopes); `InteractionService` drag mode `coaster_loop` (Shift latches the loop, X / C radius 2..6, default 3, raw keys read only during the drag); joint-driven slope / loop / cart visuals; `CoasterCartService` added only while a cart exists. Contract: [Coaster rails](COASTER_RAILS.md); evidence: [Coaster rails evidence](evidence/COASTER_RAILS.md); T160–T163 via `tools\runners\TEST_COASTER_RAILS.cmd`. The Workbench book is four pages (38 recipes). **Boundary:** no corkscrew / DNA / helix, no lateral loop offset (the loop shares its bottom row with entry and exit), no gravity or ore hauling, no cart panel, placeholder art; owner playtest and export run pending; merge after the P4 branch.
