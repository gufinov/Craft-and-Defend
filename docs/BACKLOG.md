...
**Recorded candidate result:** T27–T30 and all F0–F3 regressions pass in the provenance-matched Windows export with the matching editor closed. Day/night freezes with simulation, restores its exact saved phase and does no wall-clock catch-up. F2 captures an exact rendered viewport without pausing, persists a replacement binding across restart and resets independently. Native 3440×1440 fullscreen, centered window modes, 16:9/high-DPI/ultrawide UI captures, fixed frame/edit/save/memory measurements and a copied five-file portable folder all pass. The exact portable launcher ran without Git/tools and exited cleanly. Owner graphical acceptance is pending; do not begin P1, merge, push or publish without explicit authority.
