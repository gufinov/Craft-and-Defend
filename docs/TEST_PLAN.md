...
| T143 | P4D | A walled-in raider keeps probing (`NO_PERMITTED_ROUTE`, drill still ROUTING) instead of failing, re-plans every 2 s on its own, attacks a chest plugging the only gap as a breachable obstruction, destroys it and routes on to the core; siege machines, chests, rails and kettles are breachable structures with integrity |
