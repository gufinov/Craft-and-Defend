...
| T160 | Coaster | `rail_slope` (slope 1, not linear), `rail_loop` (loop tool, no support) and `mine_cart` (rail speed 3, `rail_mount`) are registered with icons and Workbench recipes at orders 204–206 (4 slopes, 2 loops, 1 cart) |
...
| T163 | Coaster | Visual: a size-6 loop element renders as a diameter-6 true loop with a mine cart on it, beside a slope run climbing a step with its own cart, in one 1280×720 view (`coaster-rails.png`) |
| T164 | Coaster car | `coaster_car` is a stackable (2) building item placing a rail-mounted cart entity (rail speed 3, role `rail`) with an owner-art icon and a Workbench recipe at order 207 (4 planks + 2 iron ingot + 1 castle stone → 1) |
...
| T168 | Coaster | The Lane Switcher drag ghosts four cells (entry, two middles side by side, exit one lane right) and lays them for four items; with rails before and after the twelve pieces chain; a lane-0 rail beside the lane-1 middle stays alone; the middles ride a quarter cell onto their diagonal (kettles too) and draw diagonal rails; a mine cart crosses to the far rail and back |
