...
- **Right Shift twice - Flight**: a double tap of Right Shift (within 0.4 s) lifts you off; the movement keys steer along your view (look up and press forward to climb), **Space** rises, **Z** drops, **A** flies faster, and another double tap lands you. Your body still collides with the world, and flight is refused while riding a coaster car. The first tap of the pair still performs its ordinary Interact. Raw key like V / X / C, not rebindable.
- **K - Expo Directory (Development mode only)**: opens the searchable directory of everything in the Development Expo ([Development Expo](DEVELOPMENT_EXPO.md)) - search, district / category / What's new / status filters, teleport to an exhibit and the per-subject test notes. K closes it again, as does Escape; the world keeps running behind it, as it does behind the sign editor. Pressed in an ordinary game or in CoasterCraft it does nothing but say it is a Development mode tool. Raw key like V / X / C, not rebindable, otherwise unbound. F was not available: it is `strafe_right`.
...
- **CoasterCraft mode** ([contract](COASTERCRAFT_MODE.md)): main menu **CoasterCraft** → Continue / New / Back (Escape = Back); in the mode the same keys as the game, creative placement and an infinite pack; **Escape** opens the mode's own pause menu (Resume, Save, Save and Restart, Save and Exit to Menu, Save and Quit, Settings, Keybinds, Hero armour, Track auto-clear — no drills). Launch straight into it with `START.cmd coastercraft`.
