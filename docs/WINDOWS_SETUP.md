...
This generates ignored `game\export_presets.cfg` from `game\export_presets.cfg.in`, inserts the verified local custom release-template path, and runs `--export-release "Windows Desktop"`. The portable output is `builds\CraftAndDefend\CraftAndDefend.exe` plus `CraftAndDefend.pck`, `START.cmd`, `README.txt` and the generated `build_manifest.json`. Copy those files together; the portable launcher has no Git, repository, PowerShell-script or Godot Editor dependency. In a Git checkout, the root `START.cmd` (`START.cmd build` forces a rebuild) refuses to trust a missing, unversioned, dirty or mismatched package and rebuilds it before launch; this prevents an ignored export left by an older branch from masquerading as current source. Exact hashes and runtime results are in [F0 evidence](evidence/F0_WINDOWS_INTEGRATION.md) and [launcher recovery evidence](evidence/F1_LAUNCHER_RECOVERY.md).
