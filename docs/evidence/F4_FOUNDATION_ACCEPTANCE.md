...
|---|---|---|---|---|
| T27 clock | Start at sunrise; advance only in active simulation; freeze in overlays/pause | `0800 Sunrise`; phase `0.333333 → 0.433333`; explicit pause and inventory both remained `0.433333` | PASS | `artifacts/f4-world-controls-v3-phase1.log` |
| T27 controls | Validate and apply time/cycle through the Settings accordion | Invalid `2460` changed nothing; `2200` applied as Night; disabled cycle remained frozen; re-enable succeeded | PASS | `artifacts/f4-world-controls-v3-phase1.log` |
| T27 restart | Restore phase/day/disabled toggle with no offline catch-up, then re-enable | Saved/restored phase both `0.917752866944444`, day `3`; disabled toggle remained frozen across restart; re-enable advanced | PASS | `artifacts/f4-world-controls-v3-phase2.log` |
...
| T27 night | Terrain remains visible at night | Night ambient `0.50`, dark-blue sky and terrain remain visible | PASS | `artifacts/f4-world-controls-v3-phase1.log` |
...
| T29 frame | Record fixed 60-frame graphical sample | Average `19.4246 ms`, p95 `20.257 ms`, max `20.316 ms` at 1280×720 Compatibility | PASS | `artifacts/f4-world-controls-v3-phase1.log` |
| T29 edit | Record 100 loaded-cell edit operations | `0.261 ms` total; `0.00261 ms` average; all 100 succeeded and final voxel restored | PASS | same log |
...
| T29 save | Record coherent save latency/size including cycle state | `171 ms`; `23,364` checkpoint bytes | PASS | `artifacts/f4-world-controls-v3-phase1.log` |
...
| Windows export | Matching custom release template exports the corrected committed game tree | Export PASS; corrected code commit `e52e6b7`, game tree `4354634`; graphical phase1/phase2/visual gates ran from the export | PASS | `builds/CraftAndDefend/build_manifest.json` |
...
- Corrected code commit: `e52e6b74f8cf2371b7cd5dc0fb695eab695350e4`; game tree `4354634bfe18fca71db17cec7982cada7def16b3`; rollback/accepted F3 `6e8838fc3112589f2fcf59291b02a0a040245729`.
...
- Export EXE SHA-256: `4ac128729e86108904e6d038d161322404d42d71892b1cc0dca751039aa7e4b2`; corrected export PCK SHA-256: `f4208d285dae4f247ea39e3a13368d2912ad2467919e8133197dc77c503211b0`.
