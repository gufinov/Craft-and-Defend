...
STATUS: PASS — corrected automated Windows candidate; owner fullscreen retest pending
...
| T18 | Display rollback; real window sizing; fullscreen fill; usable 16:9, ultrawide and high-DPI UI | Countdown rollback and persistence passed. Corrected export produced an actual 1600×900 window and native 3440×1440 fullscreen framebuffer; both horizontal edge pixels were game background, with no pillarboxes. Fullscreen disables/explains the windowed-size control. | PASS | `artifacts/display_fix_export_runtime.log`, `artifacts/runtime-display-fix-export/f1-fullscreen-native-fill.png`, `artifacts/display_fix_f1_phase1.log`, `artifacts/display_fix_f1_phase2.log` |
| F0 regression | Prior native shell, world, movement, editing and coherent Continue remain valid | Corrected exact export passed both F0 processes; terrain edits, dirt, transform and InputMap restored | PASS | `artifacts/display_fix_f0_phase1.log`, `artifacts/display_fix_f0_phase2.log` |
...
- Owner hands-on fullscreen retest is pending; this is not yet the accepted `main` state.
...
1. Tony performs the short fullscreen/windowed graphical retest from `START_GAME.cmd`.
...
- F1 worktree: `D:\CODEX\Craft_and_Defend\worktrees\f1-interaction`; branch `feature/f1-interaction-hardening`; original runtime checkpoint `805c3bc`; fullscreen correction `004cd34` plus the commit containing this evidence.
...
- Corrected export: `builds\CraftAndDefend\CraftAndDefend.exe` SHA-256 `4ac128729e86108904e6d038d161322404d42d71892b1cc0dca751039aa7e4b2`; `CraftAndDefend.pck` SHA-256 `14c2c83c037326b1784263e9c7c36c5e7a0775e3f4079b8fc2f97ac5a971eac0`.
- Commands: `tools/build_windows_f0.ps1`; exported `--f1-automation=display-runtime`, `phase1`, `phase2`; exported F0 `phase1`, `phase2`; `tools/validate_foundation.py`; `python -m unittest discover -s tests -v`. All corrective runtime data roots were isolated under ignored `artifacts/`.
