...
|---|---|---|---|---|
| T23 | A/B slots and repeated start/continue/save cycles remain isolated | A removed `(0,-1,38)` only; B removed `(2,-1,38)` only; each restored dirt `1` and only its own terrain edit | PASS | `artifacts/f3-export-phase1.log`, `artifacts/f3-export-phase2.log` |
...
| T25 refusal | Malformed/newer/missing-content data is clear and unchanged | Returned `MALFORMED_POINTER`, `UNSUPPORTED_SAVE_SCHEMA`, `MISSING_CONTENT_VERSION`; fixtures remained unchanged | PASS | `artifacts/f3-export-validation.log`; `T25_INVALID_REFUSAL` |
...
| Save measurements | Record size and latency in fixed scenario | F3 checkpoint bytes `23,223–23,659`; save latency `47–65 ms` | PASS | `artifacts/f3-export-phase1.log` |
| F2 regression | Progression, slots, crafting, stations and Continue remain valid | Gate and separate Continue process returned `F2_AUTOMATION_PASS T19-T22` | PASS | `artifacts/f3-regression-f2-*.log` |
| F1 regression | Keybind/settings/focus/boundary/transaction behavior remains valid | phase1/phase2 passed; `T14_PRINT_SCREEN` and ordinary `T14_FOCUS_LOSS` both PASS | PASS | `artifacts/f3-regression-f1-*.log` |
| F0 regression | Menu, ESDF, edits, rejection, coherent save/restart remain valid | phase1/phase2 result payloads report `passed:true` | PASS | `artifacts/f3-regression-f0-*.log` |
...
- Built source commit: `00d278ee3aef4093c78cf6b3fb3de5456d2f57db`; game tree `0b70ad5edbd699de90e58110b0801ab7690124df`.
...
- Export PCK: `builds\CraftAndDefend\CraftAndDefend.pck`; SHA-256 `e8c01126f777a8f8fcdd30aaaae69f9a6b4826161fab3754e54980655c28b061`.
