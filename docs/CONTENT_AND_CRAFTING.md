...
No survival hunger, damage, armor or weapons are required yet. Their eventual item definitions must use the same registry/transaction layer. Original placeholder textures are enough; preserve the numerical voxel mapping across saves. Never persist display labels as IDs.
