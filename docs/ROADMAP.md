...
| P2 — navigation risk spike | One attacker routes around, through and against edited structures | T51–T56 bounded comparison and visual diagnostic | Candidate PASS; owner review pending |
...
P1A established dependable terrain and exploration. P1B established the first original structural kit and reusable placement contract; Tony accepted its two-step stair and ordinary forward traversal on 2026-09-13. P2 now recommends a bounded project-owned snapshot/grid planner for the first P3 attacker because it can combine voxel terrain, placed entities and explicit obstruction-attack capability. The pinned experimental `VoxelAStarGrid3D` remains a much faster terrain-only benchmark, not the selected gameplay authority. Defensive mounts and one stationary weapon join P3; player equipment follows an explicit combat contract; magic remains a later reviewed track. See [Castle construction and crafting plan](CASTLE_CONSTRUCTION_AND_CRAFTING.md).
