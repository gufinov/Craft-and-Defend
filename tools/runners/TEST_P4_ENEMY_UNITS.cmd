@echo off
setlocal
title Craft and Defend - P4 Enemy Units Test
...
set "TEST_ROOT=%ROOT%artifacts\manual-p4-enemy-units-%RANDOM%%RANDOM%"
...
echo.
echo Running the enemy unit checks (orc, brute and troll models; troll ranged attack).
echo The diagnostic controls itself; it is not the playable game.
"%EXE%" --headless --log-file "%GATE_ROOT%\gate.log" -- --f0-data-root="%GATE_ROOT%" --p4-enemy-units-automation=gate
if errorlevel 1 goto :phase_failed

echo.
echo Rendering an orc, a brute and a troll side by side.
echo A game window may appear briefly and close itself.
"%EXE%" --log-file "%VISUAL_ROOT%\visual.log" -- --f0-data-root="%VISUAL_ROOT%" --p4-enemy-units-automation=visual
if errorlevel 1 goto :visual_failed

if not exist "%VISUAL_ROOT%\p4-enemy-units.png" goto :image_missing
if /i "%P4_DIAGNOSTIC_NO_OPEN%"=="1" (
  echo P4 ENEMY UNITS TEST: PASS
  echo Evidence folder: %TEST_ROOT%
  exit /b 0
)
start "" "%VISUAL_ROOT%\p4-enemy-units.png"
echo.
echo P4 ENEMY UNITS TEST: PASS
...
:phase_failed
echo.
echo P4 enemy units gate automation failed. Review: %GATE_ROOT%\gate.log
pause
exit /b 1

:visual_failed
echo.
echo P4 enemy units visual automation failed. Review: %VISUAL_ROOT%\visual.log
pause
exit /b 1
