@echo off
setlocal
title Craft and Defend - Coaster Car Test
...
set "TEST_ROOT=%ROOT%artifacts\manual-coaster-car-%RANDOM%%RANDOM%"
...
echo.
echo Running the coaster car checks (the car item, boarding and 1-9 speed, the hero model and third person).
echo The diagnostic controls itself; it is not the playable game.
"%EXE%" --headless --log-file "%GATE_ROOT%\gate.log" -- --f0-data-root="%GATE_ROOT%" --coaster-car-automation=gate
if errorlevel 1 goto :phase_failed

echo.
echo Rendering the hero riding the coaster car into a loop from the ride camera.
echo A game window may appear briefly and close itself.
"%EXE%" --log-file "%VISUAL_ROOT%\visual.log" -- --f0-data-root="%VISUAL_ROOT%" --coaster-car-automation=visual
if errorlevel 1 goto :visual_failed

if not exist "%VISUAL_ROOT%\coaster-car.png" goto :image_missing
if /i "%P4_DIAGNOSTIC_NO_OPEN%"=="1" (
  echo COASTER CAR TEST: PASS
  echo Evidence folder: %TEST_ROOT%
  exit /b 0
)
start "" "%VISUAL_ROOT%\coaster-car.png"
echo.
echo COASTER CAR TEST: PASS
...
:phase_failed
echo.
echo Coaster car gate automation failed. Review: %GATE_ROOT%\gate.log
pause
exit /b 1

:visual_failed
echo.
echo Coaster car visual automation failed. Review: %VISUAL_ROOT%\visual.log
pause
exit /b 1
