...
$portableLauncherSource = Join-Path $repositoryRoot 'tools\portable\START.cmd'
...
Copy-Item -LiteralPath $portableLauncherSource -Destination (Join-Path $buildRoot 'START.cmd') -Force
Remove-Item -LiteralPath (Join-Path $buildRoot 'START_GAME.cmd'), (Join-Path $buildRoot 'STOP_GAME.cmd') -Force -ErrorAction SilentlyContinue
...
Write-Output "Portable launcher: $(Join-Path $buildRoot 'START.cmd')"
