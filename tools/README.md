...
`build_windows_f0.ps1` creates the verified Windows export and an ignored `build_manifest.json` containing the tracked `game` tree identity, source commit, engine version and artifact hashes. `start_game.ps1` is the provenance guard used by `START.cmd` (and `-PrepareOnly` by every runner): in a Git checkout it rebuilds when the package is missing, unversioned, the tracked game tree differs, or game files are dirty. `runners\` holds the exported-game test runners (`TEST_*.cmd`, one per suite, each prints `<NAME> TEST: PASS` / `FAIL`) and `RUN_ALL.cmd` / `run_all.ps1`, which run them all with `*_DIAGNOSTIC_NO_OPEN=1` / `*_DIAGNOSTIC_NO_PAUSE=1` and print a summary table. `portable\START.cmd` is the launcher copied into the export folder (`START.cmd stop` there closes a stuck game). Use `-PrepareOnly` to validate/prepare the package without launching it.
